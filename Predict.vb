...
Imports PokemonBattlePredictor.PBP.Calculator
Imports PokemonBattlePredictor.PBP.Arena_Constants
Imports PokemonBattlePredictor.PBP.Constants.Funct_IDs
Imports PokemonBattlePredictor.PBP.InfoBlocks
Imports PokemonBattlePredictor.PBP.Tables
Imports PokemonBattlePredictor.PBP.Package
...
Namespace PBP
    ...
    Public Class Battle_Prediction : Implements Predict
            ...
            While Not battle_arena.IsBlueFainted And Not battle_arena.IsRedFainted
                ...
                If turn_queue.Count = 2 Then
                    ...
                    If poke_calc.apply_turnparalysis(first_pokemon) = False Then
                        ...
                        Dim flinch_flag As Boolean = False
                        If (first_pokemon.Team = "red" And Arena_Constants.Red_Flinch = True) Or (first_pokemon.Team = "blue" And Arena_Constants.Blue_Flinch = True) Then
                            flinch_flag = True
                            If Not Logger.isMute() Then
                                Logger.Record("I")
                                Logger.Record(first_pokemon.Name + " flinched!")
                            End If
                        End If

                        If Not first_pokemon.Status_Condition = Constants.StatusCondition.freeze And Not first_pokemon.Status_Condition = Constants.StatusCondition.sleep And Not flinch_flag Then
                        ...
                        If flinch_flag Then
                            Arena_Constants.Blue_Flinch = False
                            Arena_Constants.Red_Flinch = False
                        End If
                ...
                ElseIf turn_queue.Count = 1 Then
                    ...
                    If poke_calc.apply_turnparalysis(turn_queue.Peek()) = False Then
                        ...
                        Dim flinch_flag As Boolean = False
                        If (turn_queue.Peek().Team = "red" And Arena_Constants.Red_Flinch = True) Or (turn_queue.Peek().Team = "blue" And Arena_Constants.Blue_Flinch = True) Then
                            flinch_flag = True
                            If Not Logger.isMute() Then
                                Logger.Record("I")
                                Logger.Record(first_pokemon.Name + " flinched!")
                        ...
                        If Not turn_queue.Peek().Status_Condition = Constants.StatusCondition.freeze And Not turn_queue.Peek().Status_Condition = Constants.StatusCondition.sleep And Not flinch_flag Then
                        ...
                        If Not f_pokemon.Status_Condition = Constants.StatusCondition.freeze And Not f_pokemon.Status_Condition = Constants.StatusCondition.sleep Then
                            ...
                            If f_pokemon.Team = first_pokemon.Team Then
                                ...
                                If poke_calc.apply_confusion(f_pokemon, poke_calc) = False Then
                                    poke_calc.apply_damage(f_pokemon, turn_queue.Peek(), movetouse, poke_calc, max_or_min, arena)
                                End If
                            Else
                        ...
                        If Not s_pokemon.Status_Condition = Constants.StatusCondition.freeze And Not s_pokemon.Status_Condition = Constants.StatusCondition.sleep Then
                            ...
                            Else
                                If poke_calc.apply_confusion(s_pokemon, poke_calc) = False Then
                                    poke_calc.apply_damage(s_pokemon, f_pokemon, movetouse, poke_calc, max_or_min, arena)
                                End If
                            End If
                        End If
                    ...
                    If poke_calc.apply_turnparalysis(f_pokemon) = False Then
                        ...
                        If Not f_pokemon.Status_Condition = Constants.StatusCondition.freeze And Not f_pokemon.Status_Condition = Constants.StatusCondition.sleep Then
                            ...
                            If f_pokemon.Team = first_pokemon.Team Then
                                ...
                                REM apply_battle() determines confusion. So we do not want to calculate confusion two times.
                                If poke_calc.apply_confusion(f_pokemon, poke_calc) = False Then
                                    poke_calc.apply_damage(f_pokemon, turn_queue.Peek(), movetouse_first, poke_calc, max_or_min, arena)
                                End If
                            Else
                                REM we need to carry through apply_battle in order to let the function select the best move for us
                                'apply_battle(f_pokemon, turn_queue.Peek(), poke_calc, arena) REM causing a potential infinite loop
                                poke_calc.apply_damage(f_pokemon, turn_queue.Peek(), movetouse_second, poke_calc, max_or_min, arena)
                            End If
                    ...
                    If poke_calc.apply_turnparalysis(s_pokemon) = False Then
                        ...
                        If Not s_pokemon.Status_Condition = Constants.StatusCondition.freeze And Not s_pokemon.Status_Condition = Constants.StatusCondition.sleep Then
                            ...
                            If s_pokemon.Team = second_pokemon.Team Then
                                ...
                                poke_calc.apply_damage(s_pokemon, f_pokemon, movetouse_second, poke_calc, max_or_min, arena)
                            Else
                                If poke_calc.apply_confusion(s_pokemon, poke_calc) = False Then
                                    poke_calc.apply_damage(s_pokemon, f_pokemon, movetouse_first, poke_calc, max_or_min, arena)
                                End If
                            End If
                        End If
            ...
            Else
                ...
                If turn_poke_move2_pack.Move IsNot Nothing Then
                    If turn_poke_move2_pack.Affect_Type = Constants.Stat_Type.Affect_Opponent Then
                        poke_calc.apply_stattopokemon(second_pokemon, first_pokemon.Moves_For_Battle(turn_poke_move2_pack.Move.Name), first_pokemon.Name)
                    ElseIf turn_poke_move2_pack.Affect_Type = Constants.Stat_Type.Affect_Self Then
                        poke_calc.apply_stattopokemon(first_pokemon, first_pokemon.Moves_For_Battle(turn_poke_move2_pack.Move.Name), first_pokemon.Name)
                    Else
                        REM generally wouldn't happen but...
                        poke_calc.apply_stattopokemon(first_pokemon, first_pokemon.Moves_For_Battle(turn_poke_move2_pack.Move.Name), first_pokemon.Name)
                    End If
                ...
                Else
                    ...
                    Else
                        ...
                        If last_list.Count > 0 Then REM since this function can't be called anywhere else, this condition MUST be true at this point
                            ...
                            poke_calc.apply_damage(first_pokemon, second_pokemon, first_pokemon.Moves_For_Battle(last_list(0).Name), poke_calc, max_or_min, poke_arena)
                        Else REM in case it isn't true...
                            ...
                            REM first make sure that there are still moves left that we can use
                            If Not first_pokemon.is_nomovesleft() Then
                                Dim rand As Integer = Poke_Calculator.GenerateRandomNumber()
                                ...
                                ElseIf rand <= 100 AndAlso rand > 75 Then
                                    rand = 3
                                End If
                                poke_calc.apply_damage(first_pokemon, second_pokemon, first_pokemon.Moves_For_Battle.Item(rand), poke_calc, max_or_min, poke_arena)
                            Else
                                REM apply struggle move
                                Dim struggle As Move_Info = Move_Info.get_StruggleMove()
                                poke_calc.apply_damage(first_pokemon, second_pokemon, struggle, poke_calc, max_or_min, poke_arena)
                            End If
        ...
        Public Function FindBestMove(ByVal first_pokemon As Pokemon, ByVal second_pokemon As Pokemon, ByVal poke_calc As Poke_Calculator,
            ...
            If turn_poke_move.Name = "" Or turn_poke_move.PP <= 0 Then
                move_pack.Move = Nothing
            Else
        ...
        Public Function FindBestStatMove(ByVal first_pokemon As Pokemon, ByVal second_pokemon As Pokemon, ByVal poke_calc As Poke_Calculator,
            ...
            If off_or_def = 1 Then
                ...
                If attack_move Is Nothing Or defense_move Is Nothing Then
                    If attack_move Is Nothing And Not defense_move Is Nothing Then
                        ...
                        statmove_touse_pack.Affect_Type = Stat_Type.Affect_Opponent
                    ElseIf defense_move Is Nothing And Not attack_move Is Nothing Then
                        ...
                        statmove_touse_pack.Affect_Type = Constants.Stat_Type.Affect_Self
                    Else
                ...
                Else
                    ...
                    ElseIf attack_move.My_Turns > defense_move.My_Turns Then
                        ...
                        statmove_touse_pack.My_Turns = defense_move.My_Turns
                        statmove_touse_pack.Opponent_Turns = defense_move.Opponent_Turns
                        statmove_touse_pack.Affect_Type = Constants.Stat_Type.Affect_Opponent
                    Else
            ...
            ElseIf off_or_def = -1 Then
                ...
                Else
                    ...
                    Else
                        ...
                        statmove_touse_pack.My_Turns = defense_move.My_Turns
                        statmove_touse_pack.Opponent_Turns = defense_move.Opponent_Turns
                        statmove_touse_pack.Affect_Type = Stat_Type.Affect_Self
                    End If
            ...
            If statmove_touse_pack.Move.Name = "" Or statmove_touse_pack.Move.PP <= 0 Then
                Return Nothing
            Else
        ...
        Public Function FindBestStatMove(ByVal first_pokemon As Pokemon, ByVal second_pokemon As Pokemon, ByVal poke_calc As Poke_Calculator,
            ...
            Else
                ...
                While Not move_enum2.Current Is Nothing
                    If move_enum2.Current.PP > move_pack.Move.PP And move_enum2.Current.PP > 0 Then
                        ...
                        If move_enum2.Current.Effect.Contains("U") Then
                            move_pack.Affect_Type = Constants.Stat_Type.Affect_Self
                        ElseIf move_enum2.Current.Effect.Contains("O") Then
                            move_pack.Affect_Type = Constants.Stat_Type.Affect_Opponent
                        End If
                    End If
                    move_enum2.MoveNext()
                End While
            ...
            If Not move_pack.Move Is Nothing Then
                If move_pack.Move.Name = "" Then
                    move_pack.Move = Nothing
                End If
            End If
        ...
        Public Function Get_BestRaiseAttackStatMove(ByVal first_pokemon As Pokemon, ByVal second_pokemon As Pokemon, ByVal poke_calc As Poke_Calculator,
            ...
            While Not finalmove_enum.Current Is Nothing
                ...
                If finalmove_enum.Current.PP <= 0 Then
                    finalmove_enum.MoveNext()
                    Continue While
                End If
            ...
            If newmovestofaint = Integer.MaxValue Or statmove_touse.Name = "" Then
        ...
        Public Function Get_BestLowerAttackStatMove(ByVal first_pokemon As Pokemon, ByVal second_pokemon As Pokemon, ByVal poke_calc As Poke_Calculator,
            ...
            If newmovestofaint = Integer.MinValue Or statmove_touse.Name = "" Then
                Return Nothing
            Else
                move_package.Move = statmove_touse
                move_package.My_Turns = newmovestofaint
        ...
        Public Function FindBestStatusMove(ByVal first_pokemon As Pokemon, ByVal second_pokemon As Pokemon, ByVal poke_calc As Poke_Calculator,
            ...
            For i As Integer = 0 To listofstatus.Count - 1 Step 1
                ...
                Else
                    ...
                    poke_calc.apply_damage(f_pokemon, s_pokemon, move_enum.Current, poke_calc, max_or_min, arena)
                End If
                ...
                If s_pokemon.Status_Condition = Constants.StatusCondition.burn Or s_pokemon.Status_Condition = Constants.StatusCondition.poison Or
                    ...
                    If temp_faintturns < original_mefaint Then
                        ...
                        If return_package.Move Is Nothing Then
                            ...
                            End If
                        Else
                            If return_package.My_Turns > temp_faintturns And move_enum.Current.PP > 0 Then
                ...
                ElseIf s_pokemon.Status_Condition = Constants.StatusCondition.sleep Then
                    ...
                    If original_mefaint < original_oppfaint And original_mefaint + 3 > original_oppfaint Then
                        If return_package.Move Is Nothing Then
                            ...
                            End If
                        Else
                            If return_package.My_Turns > original_oppfaint And move_enum.Current.PP > 0 Then
                                return_package.Move = move_enum.Current
                                return_package.My_Turns = original_oppfaint
                                return_package.Opponent_Turns = original_mefaint + 3
                            End If
...
Namespace PBP.Package
    ...
    Public Class Prediction_Move_Package
        ...
        Public Property Affect_Type As Integer
            Get
                Return m_stat_affect
            End Get
            Set(value As Integer)
                m_stat_affect = value
            End Set
